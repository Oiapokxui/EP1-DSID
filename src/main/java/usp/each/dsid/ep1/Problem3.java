...
import usp.each.dsid.ep1.function.LongComparatorButItIsSerializable;
...
public class Problem3 {
    ...
    public void run() {
        ...
        final JavaRDD<Long> timeRdd = jobs.filter(str -> !str.equals(COLLECTION_HEADER))
                .map(str -> str.split(","))
                .map(arr -> arr[0])
                .map(Long::parseLong)
                .filter(time -> time != 0L)
        ...
        final Long max = timeRdd.max((new LongComparatorButItIsSerializable()));
        final Long min = timeRdd.min((new LongComparatorButItIsSerializable()));
        final int hours = (int)Math.ceil((max - min) / (double)ONE_HOUR_IN_MICROSECONDS);
